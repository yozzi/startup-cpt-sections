...
if ( !function_exists( 'cmb2_detection' ) ) {
    ...
    function cmb2_notice() {
        if ( current_user_can( 'activate_plugins' ) ) {
            echo '<div class="error message"><p>' . __( 'CMB2 plugin or StartUp Reloaded theme must be active to use custom metaboxes.', 'startup-cpt-sections' ) . '</p></div>';
        }
    }
...
if ( !function_exists( 'shortcode_ui_detection' ) ) {
    ...
    function shortcode_ui_notice() {
        if ( current_user_can( 'activate_plugins' ) ) {
            echo '<div class="error message"><p>' . __( 'Shortcake plugin must be active to use fast shortcodes.', 'startup-cpt-sections' ) . '</p></div>';
        }
    }
...
add_action( 'wp_enqueue_scripts', 'startup_cpt_sections_scripts', 15 );
?>